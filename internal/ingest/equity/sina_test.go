...
func TestParseSinaQuotes(t *testing.T) {
	defs := map[string]IndexDef{
		...
		"hf_SI":      {ID: "silver", Name: "国际白银", SinaSymbol: "hf_SI", MinPrice: 5, MaxPrice: 300},
		"hf_CL":      {ID: "crude", Name: "WTI原油", SinaSymbol: "hf_CL", MinPrice: 10, MaxPrice: 300},
	...
	var hq_str_hf_SI="75.285,,75.190,75.210,76.140,73.390,22:03:42,75.159,74.080,0,1,1,2026-05-20,纽约白银,0";
	var hq_str_hf_CL="102.172,,102.320,102.340,104.450,100.750,22:03:42,104.150,104.120,0,2,3,2026-05-20,纽约原油,0";
	...
	if rows["silver"].Price != 75.285 || rows["silver"].ChangePct <= 0 || rows["crude"].Price != 102.172 || rows["crude"].ChangePct >= 0 {
		t.Fatalf("commodity rows: %+v %+v", rows["silver"], rows["crude"])
	}
}
