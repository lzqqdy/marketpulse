...
import { fetchProviderStatus } from '@/features/market/api/providers'
import type { ProviderHealth, ProviderStatusResponse } from '@/features/market/types/providers'
...
export const useProviderStore = defineStore('providers', () => {
  ...
  const abnormalCount = computed(
    () => providers.value.filter((p) => !['healthy', 'standby', 'disabled'].includes(p.status)).length,
  )
